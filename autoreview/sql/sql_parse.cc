...
)
{
    ...
    if (global_source.dalgroup != NULL)
        strcpy(thd->thd_sinfo->dalgroup, global_source.dalgroup);
...
/*
int mysql_python_fluxdb(THD *thd)
{
     // initial Python
    ...
    json_body = (char*)my_malloc(strlen(thd->query()) + 100, MY_ZEROFILL);
    //sql_ex = sql;

    //sql = strstr(sql, "inception_magic_start");
    //sql += strlen("inception_magic_start") + 1;

    //while (my_isspace(thd->charset(), *sql))
    //{
    //    sql++;
    //}
    ...
    //strncpy(use_database, pos, (int)(pos_ex - pos) + 1);

    sprintf(json_body, "[{\"measurement\":\"dal.autoreview\",\"tags\":{\"dalgroup\": \"%s\",\"ip\":\"%s\",\"port\":%d,\"db\":\"%s\" },\"fields\":{\"wild\":%d,\"drc_check_time\":%d,\"sql\":\"%s\"}}]", 
    ...
    DBUG_ASSERT(strlen(json_body) <= strlen(thd->query()) + 100);

    Py_Initialize();
    // initial successed or not
    if (!Py_IsInitialized())
    {
       //ret = FALSE;
       //goto ERROR;
    ...
    // load python file
    pName = PyString_FromString("msginfluxdb");
    ...
    pFunc = PyDict_GetItemString(pDict, "sendfluxdb");
    ...
    PyTuple_SetItem(pArgs, 0, Py_BuildValue("s",json_body));
...
int mysql_python_mq(THD *thd)
{
    ...
    msg = (char*)my_malloc(strlen(thd->query()) + 100, MY_ZEROFILL);

    sql = thd->sql_plan_result->sql_statements;

    sprintf(msg, "{'originSql:'%s','dbName':'%s','ip':'%s','port':%d,'dalGroup':",
                        sql,
                        thd->thd_sinfo->db,
                        thd->thd_sinfo->host,
                        thd->thd_sinfo->port,
    ...
    PyRun_SimpleString("sys.path.append('./')");
    // load python file
    pName = PyString_FromString("msgrabbitmq");
    pModule = PyImport_Import(pName);
    if (!pModule)
    {
       ret = FALSE;
    ...
    pFunc = PyDict_GetItemString(pDict, "msg2mq");
    if (!pFunc || !PyCallable_Check(pFunc))
    {
        ret = FALSE;
        goto ERROR;
    ...
    PyTuple_SetItem(pArgs, 0, Py_BuildValue("s",msg));
...
int mysql_get_slave_plan_judge(THD* thd)
{
    ...
    DBUG_ENTER("mysql_get_slave_plan_judge");
    ...
    sql_plan_result = (sql_plan_result_t*)my_malloc(sizeof(sql_plan_result_t), MY_ZEROFILL);
    thd->sql_plan_result = sql_plan_result;
    if (sql_plan_result == NULL)
    {
        my_error(ER_OUTOFMEMORY, MYF(0));
        DBUG_RETURN(ER_NO);
    }
    ...
    */
    ...
    set_format_ex = set_format;
    
    pos = strstr(str, "inception_magic_start");
    pos += strlen("inception_magic_start") + 1;

    while (my_isspace(thd->charset(), *pos))
    {
        pos++;
    }
    ...
    set_format_ex = strcpy(set_format_ex, "explain ");

    set_format_ex += strlen("explain ");

    pos_ex = strstr(pos, ";");

    strncpy(set_format_ex, pos, (int)(pos_ex - pos) + 1);

    sql_plan_result->sql_statements = (char*)my_malloc((int)(pos_ex - pos) + 10,  MY_ZEROFILL);
    if (__DEBUG)
    ...
    if (strstr(sql_plan_result->sql_statements, "DRC_CHECK_TIME"))
    {
    ...
    //select databases;
    if (mysql_real_query(mysql, use_database, strlen(use_database)))
    {
        my_message(mysql_errno(mysql), mysql_error(mysql), MYF(0));
        free(set_format);
        DBUG_RETURN(ER_NO);
    ...
    */
    ...
    while(source_row)
    {
        sql_plan_result_node = (sql_plan_result_node_t*)my_malloc(sizeof(sql_plan_result_node_t), MY_ZEROFILL);
        if (sql_plan_result_node == NULL)
        {
            my_error(ER_OUTOFMEMORY, MYF(0));
            DBUG_RETURN(ER_NO);
        }
        memset(sql_plan_result_node, '\0', sizeof(sql_plan_result_node));
        if(source_row[0])
            sql_plan_result_node->id = atoi(source_row[0]); //thread id
       
        //select_typ
        if(source_row[1])
            strncpy(sql_plan_result_node->select_type, source_row[1], strlen(source_row[1]));
    
        //table
        if(source_row[2])
            strncpy(sql_plan_result_node->table, source_row[2], strlen(source_row[2]));

        //type
        if(source_row[3])
            strncpy(sql_plan_result_node->type, source_row[3], strlen(source_row[3]));

        //possible_keys
        if (source_row[4])
           strncpy(sql_plan_result_node->possible_keys, source_row[4], strlen(source_row[4]));

        //key
        if(source_row[5])
            strncpy(sql_plan_result_node->key, source_row[5], strlen(source_row[5]));

        //key_len
        if(source_row[6])
           // strncpy(sql_plan_result_node->select_type, source_row[6], strlen(source_row[6]));
           sql_plan_result_node->key_len = atoi(source_row[6]);

        //ref
        if(source_row[7])
            strncpy(sql_plan_result_node->select_type, source_row[7], strlen(source_row[7]));

        //rows
        if(source_row[8])
            sql_plan_result_node->rows = atoi(source_row[8]);

        //extra
        if(source_row[9])
            strncpy(sql_plan_result_node->extra, source_row[9], strlen(source_row[9]));
        
        LIST_ADD_LAST(link, sql_plan_result->sql_plan_lst, sql_plan_result_node);        
    ...
    mysql_deinit_sql_result_plan(thd);
    DBUG_RETURN(false);
}
...
int mysql_init_sql_cache(THD* thd)
{
    ...
    // mysql_get_slave_plan_judge(thd);
...
int mysql_deinit_sql_result_plan(THD* thd)
{
    ...
    sql_plan_result_node_t* sql_plan_result_node_next;
    DBUG_ENTER("mysql_deinit_sql_result_plan");

    sql_plan_result = thd->sql_plan_result;    

    sql_plan_result_node = LIST_GET_FIRST(sql_plan_result->sql_plan_lst);
    while (sql_plan_result_node != NULL)
    {
        sql_plan_result_node_next = LIST_GET_NEXT(link, sql_plan_result_node);
        
        my_free(sql_plan_result_node);
        if (__DEBUG)
               printf("THD:%x == sql_plan_result_node：my_free %x\n",thd,sql_plan_result->sql_statements);
        sql_plan_result_node = sql_plan_result_node_next;
    }
...
int mysql_process_command(THD *thd, Parser_state *parser_state)
{
    ...
    */
    if ((err = mysql_parse_and_check_valid(thd, parser_state)) != FALSE)
        DBUG_RETURN(err);
    DBUG_RETURN(mysql_get_slave_plan_judge(thd));
}
